...
def node2feature(node, encoding, hist_file, table_sample):
    ...
    # 检查 table_sample 是否包含所需的表数据
    if node.table_id == 0:
        sample = np.zeros(1000)  # 如果 table_id 为 0，返回一个全零的 1000 位样本
    else:
        try:
            # 尝试访问 sample
            sample = table_sample[node.query_id][node.table]
        except KeyError as e:
            # 如果发生 KeyError，打印出错信息并返回默认值
            print(f"KeyError: {e} - Missing sample for query_id {node.query_id} and table {node.table}")
            sample = np.zeros(1000)  # 返回全零的默认样本
        except Exception as e:
            # 其他异常情况，打印错误信息
            print(f"Unexpected error: {e}")
            sample = np.zeros(1000)  # 默认返回全零样本
